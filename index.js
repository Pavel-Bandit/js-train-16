...
function compareArrays(arr1, arr2) {
  ...
  const arrJson2 = JSON.stringify(arr2);
  const result = arrJson1 === arrJson2;
...
function buildMatrix(size) {
  ...
  for (let i = 0; i < size; i++) {
    const arrStringMatrix = [];
    for (let j = 0; j < size; j++) {
      arrStringMatrix.push(Math.floor(Math.random() * 10));
    }
    arrMatrix.push(arrStringMatrix);
  }
